...
def checkParams():
    ...
    parser = argparse.ArgumentParser(description='Simulazione Circuito by @Sergio L.')
    ...
    parser.add_argument('--error_rate','-r',  metavar='[0-100]', required=False, help='error rate', default=0, type=int, choices=range(0,101))
...
def main():
    ...
    if generateFiles is True:
        ...
        for input in circuit.inputs:
            ...
            print(input.name+'='+str(value), file=testFile)
        print('Generated test vector:', testVector)
    else:
        testFile.seek(0, 0)
        ...
        print('Read test vector:', testVector)
    ...
    if generateFiles is True:
        ...
        while len(errorVector) < math.floor((len(circuit.gates)-len(circuit.inputs))*errorRate/100): #fino a che gli errori non sono (massimo) ErrorRate delle porte totali
            ...
            if len(errorGate.inputs) > 0 and errorGate.name not in errorVector:
                ...
                print(errorGate.name+'='+type_error, file=errorFile)
                errorVector.append(errorGate.name+'='+type_error)
        print('Generated error vector:', errorVector)
    else:
        errorFile.seek(0, 0)
        ...
        print('Read error vector:', errorVector)

    circuit.updateValue()

    gates = list(circuit.gates)
    gates.sort(key=natural_keys)
    for key in gates:
        outputGate = circuit.gates[key]
        name = outputGate.name
        normalOutput = outputGate.output
        errorOutput = outputGate.error_output
        status = outputGate.status
        print('OUTPUT('+name+','+str(status)+')={'+str(normalOutput)+','+str(errorOutput)+'}', file=outputFile)

    print('', file=outputFile)

    outputCones = circuit.getOutputConesNO()
    for cone in outputCones:
        name = cone[0]
        gates = ','.join(cone[1])
        print(name+'{'+gates+'}', file=outputFile)
    return
